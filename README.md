...
The goal of `appsflyeR` is to help `R` users to access digital marketing data from AppsFlyer via `Windsor.ai` `API` in a convenient way from `R`.
...
You can install the development version of appsflyeR from [GitHub](https://github.com/) with:

``` r
# install.packages("devtools")
devtools::install_github("pablosanchezmart/appsflyeR")
```
...
You need to get a free API key to access windsor.ai's APIs. Register your account first and add a data source like AppsFlyer and then get the API key. For more details check out our official API documentation and this article. Get the API key at https://onboard.windsor.ai. You have a 30 days trial for free.

## Example

The package currently has only one function `fetch_appsflyer` which will return a `data.frame` provided that all of the arguments are supplied to it: 
...
Will return a `data.frame` with marketing data from AppsFlyer.  
